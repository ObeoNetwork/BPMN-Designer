...
public class CategoryPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public CategoryPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject category, String editing_mode) {
		...
		partKey = Bpmn2ViewsRepository.Category.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (category.getId() != null && isAccessible(Bpmn2ViewsRepository.Category.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, category.getId()));
			
			if (isAccessible(Bpmn2ViewsRepository.Category.Properties.extensionDefinitions)) {
				extensionDefinitionsSettings = new ReferencesTableSettings(category, Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions());
				basePart.initExtensionDefinitions(extensionDefinitionsSettings);
			}
			if (isAccessible(Bpmn2ViewsRepository.Category.Properties.extensionValues)) {
				extensionValuesSettings = new ReferencesTableSettings(category, Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues());
				basePart.initExtensionValues(extensionValuesSettings);
			}
			if (isAccessible(Bpmn2ViewsRepository.Category.Properties.documentation)) {
				documentationSettings = new ReferencesTableSettings(category, Bpmn2Package.eINSTANCE.getBaseElement_Documentation());
				basePart.initDocumentation(documentationSettings);
			}
			if (category.getName() != null && isAccessible(Bpmn2ViewsRepository.Category.Properties.name))
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == Bpmn2ViewsRepository.Category.Properties.id) {
			return Bpmn2Package.eINSTANCE.getBaseElement_Id();
		}
		if (editorKey == Bpmn2ViewsRepository.Category.Properties.extensionDefinitions) {
			return Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions();
		}
		if (editorKey == Bpmn2ViewsRepository.Category.Properties.extensionValues) {
			return Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues();
		}
		if (editorKey == Bpmn2ViewsRepository.Category.Properties.documentation) {
			return Bpmn2Package.eINSTANCE.getBaseElement_Documentation();
		}
		if (editorKey == Bpmn2ViewsRepository.Category.Properties.name) {
			return Bpmn2Package.eINSTANCE.getCategory_Name();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (Bpmn2ViewsRepository.Category.Properties.id == event.getAffectedEditor()) {
			category.setId((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		if (Bpmn2ViewsRepository.Category.Properties.extensionDefinitions == event.getAffectedEditor()) {
		...
		if (Bpmn2ViewsRepository.Category.Properties.extensionValues == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (Bpmn2ViewsRepository.Category.Properties.documentation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (Bpmn2ViewsRepository.Category.Properties.name == event.getAffectedEditor()) {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (Bpmn2Package.eINSTANCE.getBaseElement_Id().equals(msg.getFeature()) && basePart != null && isAccessible(Bpmn2ViewsRepository.Category.Properties.id)) {
			...
			if (Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions().equals(msg.getFeature())  && isAccessible(Bpmn2ViewsRepository.Category.Properties.extensionDefinitions))
				basePart.updateExtensionDefinitions();
			if (Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues().equals(msg.getFeature()) && isAccessible(Bpmn2ViewsRepository.Category.Properties.extensionValues))
				basePart.updateExtensionValues();
			if (Bpmn2Package.eINSTANCE.getBaseElement_Documentation().equals(msg.getFeature()) && isAccessible(Bpmn2ViewsRepository.Category.Properties.documentation))
				basePart.updateDocumentation();
			if (Bpmn2Package.eINSTANCE.getCategory_Name().equals(msg.getFeature()) && basePart != null && isAccessible(Bpmn2ViewsRepository.Category.Properties.name)) {
				if (msg.getNewValue() != null) {
	...
	public boolean isRequired(Object key, int kind) {
		return key == Bpmn2ViewsRepository.Category.Properties.id || key == Bpmn2ViewsRepository.Category.Properties.name;
	}
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (Bpmn2ViewsRepository.Category.Properties.name == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(Bpmn2Package.eINSTANCE.getCategory_Name().getEAttributeType(), (String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(Bpmn2Package.eINSTANCE.getCategory_Name().getEAttributeType(), newValue);
				}
