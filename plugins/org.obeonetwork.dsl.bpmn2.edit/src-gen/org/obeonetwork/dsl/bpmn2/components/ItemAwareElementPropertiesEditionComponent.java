 ...
 * 
 */
public class ItemAwareElementPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public ItemAwareElementPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject itemAwareElement, String editing_mode) {
		super(editingContext, itemAwareElement, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = Bpmn2ViewsRepository.class;
		partKey = Bpmn2ViewsRepository.ItemAwareElement.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final ItemAwareElement itemAwareElement = (ItemAwareElement)elt;
			final ItemAwareElementPropertiesEditionPart basePart = (ItemAwareElementPropertiesEditionPart)editingPart;
			// init values
			if (itemAwareElement.getId() != null && isAccessible(Bpmn2ViewsRepository.ItemAwareElement.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, itemAwareElement.getId()));
			
			if (isAccessible(Bpmn2ViewsRepository.ItemAwareElement.Properties.extensionDefinitions)) {
				extensionDefinitionsSettings = new ReferencesTableSettings(itemAwareElement, Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions());
				basePart.initExtensionDefinitions(extensionDefinitionsSettings);
			}
			if (isAccessible(Bpmn2ViewsRepository.ItemAwareElement.Properties.extensionValues)) {
				extensionValuesSettings = new ReferencesTableSettings(itemAwareElement, Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues());
				basePart.initExtensionValues(extensionValuesSettings);
			}
			if (isAccessible(Bpmn2ViewsRepository.ItemAwareElement.Properties.documentation)) {
				documentationSettings = new ReferencesTableSettings(itemAwareElement, Bpmn2Package.eINSTANCE.getBaseElement_Documentation());
				basePart.initDocumentation(documentationSettings);
			}
			if (isAccessible(Bpmn2ViewsRepository.ItemAwareElement.Properties.itemSubjectRef)) {
				// init part
				itemSubjectRefSettings = new EObjectFlatComboSettings(itemAwareElement, Bpmn2Package.eINSTANCE.getItemAwareElement_ItemSubjectRef());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == Bpmn2ViewsRepository.ItemAwareElement.Properties.id) {
			return Bpmn2Package.eINSTANCE.getBaseElement_Id();
		}
		if (editorKey == Bpmn2ViewsRepository.ItemAwareElement.Properties.extensionDefinitions) {
			return Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions();
		}
		if (editorKey == Bpmn2ViewsRepository.ItemAwareElement.Properties.extensionValues) {
			return Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues();
		}
		if (editorKey == Bpmn2ViewsRepository.ItemAwareElement.Properties.documentation) {
			return Bpmn2Package.eINSTANCE.getBaseElement_Documentation();
		}
		if (editorKey == Bpmn2ViewsRepository.ItemAwareElement.Properties.itemSubjectRef) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		ItemAwareElement itemAwareElement = (ItemAwareElement)semanticObject;
		if (Bpmn2ViewsRepository.ItemAwareElement.Properties.id == event.getAffectedEditor()) {
			itemAwareElement.setId((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		if (Bpmn2ViewsRepository.ItemAwareElement.Properties.extensionDefinitions == event.getAffectedEditor()) {
		...
		if (Bpmn2ViewsRepository.ItemAwareElement.Properties.extensionValues == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (Bpmn2ViewsRepository.ItemAwareElement.Properties.documentation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (Bpmn2ViewsRepository.ItemAwareElement.Properties.itemSubjectRef == event.getAffectedEditor()) {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			ItemAwareElementPropertiesEditionPart basePart = (ItemAwareElementPropertiesEditionPart)editingPart;
			if (Bpmn2Package.eINSTANCE.getBaseElement_Id().equals(msg.getFeature()) && basePart != null && isAccessible(Bpmn2ViewsRepository.ItemAwareElement.Properties.id)) {
			...
			if (Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions().equals(msg.getFeature())  && isAccessible(Bpmn2ViewsRepository.ItemAwareElement.Properties.extensionDefinitions))
				basePart.updateExtensionDefinitions();
			if (Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues().equals(msg.getFeature()) && isAccessible(Bpmn2ViewsRepository.ItemAwareElement.Properties.extensionValues))
				basePart.updateExtensionValues();
			if (Bpmn2Package.eINSTANCE.getBaseElement_Documentation().equals(msg.getFeature()) && isAccessible(Bpmn2ViewsRepository.ItemAwareElement.Properties.documentation))
				basePart.updateDocumentation();
			if (Bpmn2Package.eINSTANCE.getItemAwareElement_ItemSubjectRef().equals(msg.getFeature()) && basePart != null && isAccessible(Bpmn2ViewsRepository.ItemAwareElement.Properties.itemSubjectRef))
	...
	public boolean isRequired(Object key, int kind) {
		return key == Bpmn2ViewsRepository.ItemAwareElement.Properties.id;
	}
