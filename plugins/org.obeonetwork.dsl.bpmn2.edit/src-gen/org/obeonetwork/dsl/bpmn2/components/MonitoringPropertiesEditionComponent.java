 ...
 * 
 */
public class MonitoringPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public MonitoringPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject monitoring, String editing_mode) {
		super(editingContext, monitoring, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = Bpmn2ViewsRepository.class;
		partKey = Bpmn2ViewsRepository.Monitoring.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final Monitoring monitoring = (Monitoring)elt;
			final MonitoringPropertiesEditionPart basePart = (MonitoringPropertiesEditionPart)editingPart;
			// init values
			if (monitoring.getId() != null && isAccessible(Bpmn2ViewsRepository.Monitoring.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, monitoring.getId()));
			
			if (isAccessible(Bpmn2ViewsRepository.Monitoring.Properties.extensionDefinitions)) {
				extensionDefinitionsSettings = new ReferencesTableSettings(monitoring, Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions());
				basePart.initExtensionDefinitions(extensionDefinitionsSettings);
			}
			if (isAccessible(Bpmn2ViewsRepository.Monitoring.Properties.extensionValues)) {
				extensionValuesSettings = new ReferencesTableSettings(monitoring, Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues());
				basePart.initExtensionValues(extensionValuesSettings);
			}
			if (isAccessible(Bpmn2ViewsRepository.Monitoring.Properties.documentation)) {
				documentationSettings = new ReferencesTableSettings(monitoring, Bpmn2Package.eINSTANCE.getBaseElement_Documentation());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == Bpmn2ViewsRepository.Monitoring.Properties.id) {
			return Bpmn2Package.eINSTANCE.getBaseElement_Id();
		}
		if (editorKey == Bpmn2ViewsRepository.Monitoring.Properties.extensionDefinitions) {
			return Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions();
		}
		if (editorKey == Bpmn2ViewsRepository.Monitoring.Properties.extensionValues) {
			return Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues();
		}
		if (editorKey == Bpmn2ViewsRepository.Monitoring.Properties.documentation) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		Monitoring monitoring = (Monitoring)semanticObject;
		if (Bpmn2ViewsRepository.Monitoring.Properties.id == event.getAffectedEditor()) {
			monitoring.setId((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		if (Bpmn2ViewsRepository.Monitoring.Properties.extensionDefinitions == event.getAffectedEditor()) {
		...
		if (Bpmn2ViewsRepository.Monitoring.Properties.extensionValues == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (Bpmn2ViewsRepository.Monitoring.Properties.documentation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			MonitoringPropertiesEditionPart basePart = (MonitoringPropertiesEditionPart)editingPart;
			if (Bpmn2Package.eINSTANCE.getBaseElement_Id().equals(msg.getFeature()) && basePart != null && isAccessible(Bpmn2ViewsRepository.Monitoring.Properties.id)) {
			...
			if (Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions().equals(msg.getFeature())  && isAccessible(Bpmn2ViewsRepository.Monitoring.Properties.extensionDefinitions))
				basePart.updateExtensionDefinitions();
			if (Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues().equals(msg.getFeature()) && isAccessible(Bpmn2ViewsRepository.Monitoring.Properties.extensionValues))
				basePart.updateExtensionValues();
			if (Bpmn2Package.eINSTANCE.getBaseElement_Documentation().equals(msg.getFeature()) && isAccessible(Bpmn2ViewsRepository.Monitoring.Properties.documentation))
	...
	public boolean isRequired(Object key, int kind) {
		return key == Bpmn2ViewsRepository.Monitoring.Properties.id;
	}
