 ...
 * 
 */
public class EndPointPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public EndPointPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject endPoint, String editing_mode) {
		super(editingContext, endPoint, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = Bpmn2ViewsRepository.class;
		partKey = Bpmn2ViewsRepository.EndPoint.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final EndPoint endPoint = (EndPoint)elt;
			final EndPointPropertiesEditionPart basePart = (EndPointPropertiesEditionPart)editingPart;
			// init values
			if (endPoint.getId() != null && isAccessible(Bpmn2ViewsRepository.EndPoint.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, endPoint.getId()));
			
			if (isAccessible(Bpmn2ViewsRepository.EndPoint.Properties.extensionDefinitions)) {
				extensionDefinitionsSettings = new ReferencesTableSettings(endPoint, Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions());
				basePart.initExtensionDefinitions(extensionDefinitionsSettings);
			}
			if (isAccessible(Bpmn2ViewsRepository.EndPoint.Properties.extensionValues)) {
				extensionValuesSettings = new ReferencesTableSettings(endPoint, Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues());
				basePart.initExtensionValues(extensionValuesSettings);
			}
			if (isAccessible(Bpmn2ViewsRepository.EndPoint.Properties.documentation)) {
				documentationSettings = new ReferencesTableSettings(endPoint, Bpmn2Package.eINSTANCE.getBaseElement_Documentation());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == Bpmn2ViewsRepository.EndPoint.Properties.id) {
			return Bpmn2Package.eINSTANCE.getBaseElement_Id();
		}
		if (editorKey == Bpmn2ViewsRepository.EndPoint.Properties.extensionDefinitions) {
			return Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions();
		}
		if (editorKey == Bpmn2ViewsRepository.EndPoint.Properties.extensionValues) {
			return Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues();
		}
		if (editorKey == Bpmn2ViewsRepository.EndPoint.Properties.documentation) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		EndPoint endPoint = (EndPoint)semanticObject;
		if (Bpmn2ViewsRepository.EndPoint.Properties.id == event.getAffectedEditor()) {
			endPoint.setId((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		if (Bpmn2ViewsRepository.EndPoint.Properties.extensionDefinitions == event.getAffectedEditor()) {
		...
		if (Bpmn2ViewsRepository.EndPoint.Properties.extensionValues == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (Bpmn2ViewsRepository.EndPoint.Properties.documentation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			EndPointPropertiesEditionPart basePart = (EndPointPropertiesEditionPart)editingPart;
			if (Bpmn2Package.eINSTANCE.getBaseElement_Id().equals(msg.getFeature()) && basePart != null && isAccessible(Bpmn2ViewsRepository.EndPoint.Properties.id)) {
			...
			if (Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions().equals(msg.getFeature())  && isAccessible(Bpmn2ViewsRepository.EndPoint.Properties.extensionDefinitions))
				basePart.updateExtensionDefinitions();
			if (Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues().equals(msg.getFeature()) && isAccessible(Bpmn2ViewsRepository.EndPoint.Properties.extensionValues))
				basePart.updateExtensionValues();
			if (Bpmn2Package.eINSTANCE.getBaseElement_Documentation().equals(msg.getFeature()) && isAccessible(Bpmn2ViewsRepository.EndPoint.Properties.documentation))
	...
	public boolean isRequired(Object key, int kind) {
		return key == Bpmn2ViewsRepository.EndPoint.Properties.id;
	}
