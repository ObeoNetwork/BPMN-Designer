 ...
 * 
 */
public class RenderingPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public RenderingPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject rendering, String editing_mode) {
		super(editingContext, rendering, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = Bpmn2ViewsRepository.class;
		partKey = Bpmn2ViewsRepository.Rendering.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final Rendering rendering = (Rendering)elt;
			final RenderingPropertiesEditionPart basePart = (RenderingPropertiesEditionPart)editingPart;
			// init values
			if (rendering.getId() != null && isAccessible(Bpmn2ViewsRepository.Rendering.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, rendering.getId()));
			
			if (isAccessible(Bpmn2ViewsRepository.Rendering.Properties.extensionDefinitions)) {
				extensionDefinitionsSettings = new ReferencesTableSettings(rendering, Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions());
				basePart.initExtensionDefinitions(extensionDefinitionsSettings);
			}
			if (isAccessible(Bpmn2ViewsRepository.Rendering.Properties.extensionValues)) {
				extensionValuesSettings = new ReferencesTableSettings(rendering, Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues());
				basePart.initExtensionValues(extensionValuesSettings);
			}
			if (isAccessible(Bpmn2ViewsRepository.Rendering.Properties.documentation)) {
				documentationSettings = new ReferencesTableSettings(rendering, Bpmn2Package.eINSTANCE.getBaseElement_Documentation());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == Bpmn2ViewsRepository.Rendering.Properties.id) {
			return Bpmn2Package.eINSTANCE.getBaseElement_Id();
		}
		if (editorKey == Bpmn2ViewsRepository.Rendering.Properties.extensionDefinitions) {
			return Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions();
		}
		if (editorKey == Bpmn2ViewsRepository.Rendering.Properties.extensionValues) {
			return Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues();
		}
		if (editorKey == Bpmn2ViewsRepository.Rendering.Properties.documentation) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		Rendering rendering = (Rendering)semanticObject;
		if (Bpmn2ViewsRepository.Rendering.Properties.id == event.getAffectedEditor()) {
			rendering.setId((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		if (Bpmn2ViewsRepository.Rendering.Properties.extensionDefinitions == event.getAffectedEditor()) {
		...
		if (Bpmn2ViewsRepository.Rendering.Properties.extensionValues == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (Bpmn2ViewsRepository.Rendering.Properties.documentation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			RenderingPropertiesEditionPart basePart = (RenderingPropertiesEditionPart)editingPart;
			if (Bpmn2Package.eINSTANCE.getBaseElement_Id().equals(msg.getFeature()) && basePart != null && isAccessible(Bpmn2ViewsRepository.Rendering.Properties.id)) {
			...
			if (Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions().equals(msg.getFeature())  && isAccessible(Bpmn2ViewsRepository.Rendering.Properties.extensionDefinitions))
				basePart.updateExtensionDefinitions();
			if (Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues().equals(msg.getFeature()) && isAccessible(Bpmn2ViewsRepository.Rendering.Properties.extensionValues))
				basePart.updateExtensionValues();
			if (Bpmn2Package.eINSTANCE.getBaseElement_Documentation().equals(msg.getFeature()) && isAccessible(Bpmn2ViewsRepository.Rendering.Properties.documentation))
	...
	public boolean isRequired(Object key, int kind) {
		return key == Bpmn2ViewsRepository.Rendering.Properties.id;
	}
