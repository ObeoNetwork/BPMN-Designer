...
public class AssignmentPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public AssignmentPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject assignment, String editing_mode) {
		...
		partKey = Bpmn2ViewsRepository.Assignment.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (assignment.getId() != null && isAccessible(Bpmn2ViewsRepository.Assignment.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, assignment.getId()));
			
			if (isAccessible(Bpmn2ViewsRepository.Assignment.Properties.extensionDefinitions)) {
				extensionDefinitionsSettings = new ReferencesTableSettings(assignment, Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions());
				basePart.initExtensionDefinitions(extensionDefinitionsSettings);
			}
			if (isAccessible(Bpmn2ViewsRepository.Assignment.Properties.extensionValues)) {
				extensionValuesSettings = new ReferencesTableSettings(assignment, Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues());
				basePart.initExtensionValues(extensionValuesSettings);
			}
			if (isAccessible(Bpmn2ViewsRepository.Assignment.Properties.documentation)) {
				documentationSettings = new ReferencesTableSettings(assignment, Bpmn2Package.eINSTANCE.getBaseElement_Documentation());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == Bpmn2ViewsRepository.Assignment.Properties.id) {
			return Bpmn2Package.eINSTANCE.getBaseElement_Id();
		}
		if (editorKey == Bpmn2ViewsRepository.Assignment.Properties.extensionDefinitions) {
			return Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions();
		}
		if (editorKey == Bpmn2ViewsRepository.Assignment.Properties.extensionValues) {
			return Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues();
		}
		if (editorKey == Bpmn2ViewsRepository.Assignment.Properties.documentation) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (Bpmn2ViewsRepository.Assignment.Properties.id == event.getAffectedEditor()) {
			assignment.setId((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		if (Bpmn2ViewsRepository.Assignment.Properties.extensionDefinitions == event.getAffectedEditor()) {
		...
		if (Bpmn2ViewsRepository.Assignment.Properties.extensionValues == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (Bpmn2ViewsRepository.Assignment.Properties.documentation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (Bpmn2Package.eINSTANCE.getBaseElement_Id().equals(msg.getFeature()) && basePart != null && isAccessible(Bpmn2ViewsRepository.Assignment.Properties.id)) {
			...
			if (Bpmn2Package.eINSTANCE.getBaseElement_ExtensionDefinitions().equals(msg.getFeature())  && isAccessible(Bpmn2ViewsRepository.Assignment.Properties.extensionDefinitions))
				basePart.updateExtensionDefinitions();
			if (Bpmn2Package.eINSTANCE.getBaseElement_ExtensionValues().equals(msg.getFeature()) && isAccessible(Bpmn2ViewsRepository.Assignment.Properties.extensionValues))
				basePart.updateExtensionValues();
			if (Bpmn2Package.eINSTANCE.getBaseElement_Documentation().equals(msg.getFeature()) && isAccessible(Bpmn2ViewsRepository.Assignment.Properties.documentation))
	...
	public boolean isRequired(Object key, int kind) {
		return key == Bpmn2ViewsRepository.Assignment.Properties.id;
	}
