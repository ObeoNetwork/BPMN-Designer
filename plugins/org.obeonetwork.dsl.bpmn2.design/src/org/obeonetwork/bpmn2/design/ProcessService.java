...
public class ProcessService {
    ...
    public static void paste(EObject container, EObject containerView, EObject copiedView, EObject copiedElement) {
        ...
        for (EReference containment : container.eClass().getEAllContainments()) {
            if (containment.getEType().equals(copiedElement.eClass()) || copiedElement.eClass().getEAllSuperTypes().contains(containment.getEType())) {
    ...
    private boolean isEventTaskOrGateway(EObject element) {
        return element instanceof BoundaryEvent || element instanceof ImplicitThrowEvent || element instanceof IntermediateCatchEvent || element instanceof EndEvent || element instanceof StartEvent
                || element instanceof Task || element instanceof ParallelGateway || element instanceof ExclusiveGateway || element instanceof InclusiveGateway || element instanceof ComplexGateway
